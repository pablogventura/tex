...
class XmlBuilder:
    ...
    def __init__(self):
        self.root = xml.etree.ElementTree.Element(
            'lx:lyx', {'xmlns:lx': 'http://getfo.org/lyxml/'})
        self.node = self.root
        self.stack = []
    ...
    def opt_with_prefix(self, opts):
        ...
        for (k, v) in opts.iteritems():
            ...
            new_opts['lx:' + xml_safe_name(k.strip())] = v
        return new_opts
    ...
    def begin_layout(self, lname, opts):
        ...
        node = xml.etree.ElementTree.Element(
            xml_safe_name(lname), self.opt_with_prefix(opts))
    ...
    for l in h:
        ...
        if '\\end_header' == l2:
            ...
            for kid in root.findall('.//{http://getfo.org/lyxml/}branch'):
                branch_name = lyx_safe_string(
                    kid.get('{http://getfo.org/lyxml/}ann', ''))
...
def x2l_layout(node, force_name, attr_from_inset, h_out):
    ...
    for (k, v) in xml_attr.iteritems():
        if '{http://getfo.org/lyxml/}id' == k:
            ...
            fake_node = xml.etree.ElementTree.Element(
                '{http://getfo.org/lyxml/}commandinset', label_attr)
        else:
...
inset_param_order = [
    '{http://getfo.org/lyxml/}' + s for s in inset_param_order]
...
def x2l_inset(node, h_out):
    ...
    if 'branch' == gi:
        ...
        node.attrib.setdefault('{http://getfo.org/lyxml/}reference', '"' + lyx_safe_string(
            node.attrib.get('{http://getfo.org/lyxml/}idref', "")) + '"')
        ...
        if '{http://getfo.org/lyxml/}features' == kid_tbl.tag:
            ...
            for kid_row in kid_tbl.getchildren():
                if '{http://getfo.org/lyxml/}cell' == kid_row.tag:
                    i_col = i_col + 1
                    if i_col > n_cols:
                        # error message displayed later in check i_col !=
                        # n_cols
                        break
                    on_cell(kid_row)
                else:
...
if not(options.l2x) and not(options.x2l):
    parser.error(
        "no --lyx2xml or --xml2lyx is given, and can\'t guess direction")
    sys.exit(-1)
if out_file is None:
    out_file = '-'
if in_file is None:
